...
RUN cmake -B build && cmake --build build --target example-cuda-xmake --config Release --parallel 8

# DEPLOYING
FROM rockylinux:9-minimal

COPY --from=builder /app/build/example-cuda-xmake /example-cuda-xmake

CMD ["/example-cuda-xmake"]
